...
class AnimalDataFrame:
    ...
    def _get_motion_df(self) -> (DataFrame, DataFrame, list[str]):
        """
        ...
        if self._window_size == 1:
            ...
            keys = position_df.keys()[:-self._window_size + 1]
        ...
        for kp in kp_in_df:
            # u = 0
            for key in keys:
                key_upper = key + self._window_size
                ...
                if not p_start.isnull().any():
                    if position_df.keys().__contains__(key_upper):
                        ...
                        if not p_end.isnull().any():
                            ...
                            if velocity_df.keys().__contains__(key - self._window_size):
                                u = velocity_df.loc[kp, key - self._window_size]
                                dt = self._window_size
                            else:
                                u = np.nan
